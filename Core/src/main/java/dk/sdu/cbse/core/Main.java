...
public class Main extends Application {
    ...
    public void start(Stage window) throws Exception {
        ...
        for (IGameService iGamePlugin : getPluginServices()) {
    ...
    private Collection<? extends IGameService> getPluginServices() {
        return ServiceLoader.load(IGameService.class).stream().map(ServiceLoader.Provider::get).collect(toList());
    }

    private Collection<? extends IEntityService> getEntityProcessingServices() {
        return ServiceLoader.load(IEntityService.class).stream().map(ServiceLoader.Provider::get).collect(toList());
    }

    private Collection<? extends IPostService> getPostEntityProcessingServices() {
